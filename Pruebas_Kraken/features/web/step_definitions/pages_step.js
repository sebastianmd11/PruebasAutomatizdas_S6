...
When('I write the New Page´s title {string}', async function (title) {
    let element = await this.driver.$('textarea[placeholder="Page title"]');
    return await element.setValue(title);
});

When('I write the New Page´s body {string}', async function (body) {
    let element = await this.driver.$('div[data-placeholder="Begin writing your page..."]');
    return await element.setValue(body);
});


Then('I verify there is a new page named {string}', async function(name) {
...
When('I click on button {string}', async function (buttonName) {
...
Then('the deleted page {string} should not exist', async function (deletedPageTitle) {
    const elements = await this.driver.$$('h3.gh-content-entry-title');
    
    for (let i = 0; i < elements.length; i++) {
        const text = await elements[i].getText();
        assert.notStrictEqual(text, deletedPageTitle);
    }
});
